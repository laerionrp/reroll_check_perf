...
function catalogueGetFilteredVehicles() {
  ...
  return catalogueState.vehicles.filter(vehicle => {
    const searchable = [vehicle.name, vehicle.category, vehicle.manufacturer, vehicle.dealership_id].map(catalogueNormalize).join(' ');
    return (!query || searchable.includes(query)) &&
      (!catalogueState.filters.category || catalogueNormalize(vehicle.category) === catalogueState.filters.category) &&
      (!catalogueState.filters.dealership || catalogueNormalize(vehicle.dealership_id) === catalogueState.filters.dealership) &&
...
function catalogueRenderFilters() {
  document.querySelectorAll('.catalogue-filter').forEach(filter => {
    ...
    const values = [...new Set(catalogueState.vehicles.map(vehicle => String(vehicle[field === 'manufacturer' ? 'manufacturer' : field === 'dealership' ? 'dealership_id' : 'category'] || '').trim()).filter(Boolean))]
...
function catalogueRenderList() {
  ...
  vehicles.forEach(vehicle => {
    ...
    item.type = 'button';
    item.className = 'catalogue-vehicle-item ' + (String(vehicle.id) === catalogueState.selectedId ? 'selected ' : '') + catalogueDealerClass(vehicle.dealership_id);
    item.setAttribute('role', 'option');
    item.setAttribute('aria-selected', String(vehicle.id) === catalogueState.selectedId ? 'true' : 'false');
    item.innerHTML = `<strong>${catalogueEscape(vehicle.name)}</strong><span>${catalogueEscape(vehicle.category || 'Catégorie non renseignée')}</span><small>${catalogueEscape(catalogueDisplayDealership(vehicle.dealership_id))}</small>`;
...
function catalogueRenderDetail(vehicle) {
  ...
  catalogueElements.detail.className = 'catalogue-vehicle-detail ' + catalogueDealerClass(vehicle.dealership_id);
  ...
  catalogueElements.dealership.textContent = catalogueDisplayDealership(vehicle.dealership_id);
